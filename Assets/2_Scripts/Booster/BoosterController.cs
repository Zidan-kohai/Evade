...
public class BoosterController : MonoBehaviour {
    ...
    private void Awake() {
        ...
        for (int i = 0; i < geekPlayData.CurrentBoosterKeys.Count; i++) {
            ...
            for (int j = i; j < boosterItem.Count; j++) {
                ...
                foreach (MyDictionary booster in Boosters) {
                    if (geekPlayData.CurrentBoosterKeys[i] == booster.data.indexOnPlayer && 
                        geekPlayData.BuyedBoosterID.GetByKey(booster.data.indexOnPlayer).value != 0) 
                    {
                        ...
                        boosterItem[j].boostEvent.AddListener(() => {
                            if(geekPlayData.BuyedBoosterID.GetByKey(booster.data.indexOnPlayer).value == 0) {
                                return;
                            }
                            
                            if(booster.boosterEvent?.Invoke(booster.data) == true) {
                                geekPlayData.BuyedBoosterID.GetByKey(booster.data.indexOnPlayer).value--;
                                boosterItem[j].remainingCounView.text = geekPlayData.BuyedBoosterID.GetByKey(booster.data.indexOnPlayer).value.ToString();
                            }
        ...
        foreach (MyDictionary booster in Boosters) {
            if (geekPlayData.CurrentEquipedItemID == booster.data.indexOnPlayer && booster.data.type == SubjectType.Item) {
                ...
                boosterItem[5].boostEvent.AddListener(() => {
                    ...
                    if(meatUsedCount <= 0) { 
                        boosterItem[5].gameObject?.SetActive(false); 
                    }
                });
                break;
            }
    ...
    private void Update() {
        ...
        // слишком много условий. вынеси логику в отдельный метод
    ...
    public bool WandOfLightBoost(ShopItemData data) {
        Instantiate(
            WandOfLightPrefab, 
            realyPlayerTransform.position + realyPlayerTransform.forward * 5 + new Vector3(0, 5, 0), 
            Quaternion.identity, 
            null
        );
